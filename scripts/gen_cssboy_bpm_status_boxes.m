function gen_cssboy_bpm_status_boxes(areas)

ncrates = size(areas,1);
nbpmslots = size(areas, 2);

wuid_counter = 0;
fid = fopen('template_bpm_opi_gui.xml','w');
for crate_number = 1:ncrates
    for bpmslot_number = 1:nbpmslots
        fprintf(fid, '  <!-- Generated BPM Status Rectangle widget - Crate %d - Slot %d -->\n', crate_number, bpmslot_number);
        ...
        fprintf(fid, '    <x>%d</x>\n', 200+29*(crate_number-1));
        fprintf(fid, '    <name>Rectangle_BPM_status_%d_%d</name>\n', crate_number, bpmslot_number);
